...
static mcp2515_status_t mcp2515_spi_interface(mcp2515_transmission_mode_t trans_mode, uint16_t cmd, uint8_t addr, uint32_t* data, uint32_t data_len) {
    ...
    spi_config_t spi_trans;

    spi_trans.cmd = cmd;
    spi_trans.data_len = data_len;

    if(addr != 255) {
        spi_trans.cmd_len = 16;
        spi_trans.addr = addr;
    }
	...
	uint8_t* spi_data;
	uint16_t total_len;
	total_len = spi_trans.cmd_len + spi_trans.data_len;
	spi_data = (uint8_t*)malloc( (total_len / 8) * sizeof(uint8_t) );
	spi_trans.data = spi_data;
	spi_trans.spi_mode = SPI_SEND;
	for(int i = 0; i < (data_len / 8); i++) {	//since data is stored LSB from uint8_t to uint32_t, the order of data must be flipped around
	    *(spi_data + (data_len / 8) - 1 - i) = ( ( *(data) & ( 0xFF << (i * 8) ) ) >> (i * 8) );
	}
	spi_master_transmit(spi_trans);
	free(spi_data);
    }
    else {
	...
	spi_trans.spi_mode = SPI_RECV;
	spi_trans.data = (uint8_t*)data;
	spi_master_transmit(spi_trans);
	while (j > -1) {			//since data is stored LSB from uint8_t to uint32_t, the order of data must be flipped around
	    *(data + k) = ( ((*(data + k) & 0xFF) << 24) | ((*(data + k) & (0xFF << 24)) >> 24) | ((*(data + k) & (0xFF << 8)) << 8) | ((*(data + k) & (0xFF << 16)) >> 8) );
	    k++;
	    j--;
	}
